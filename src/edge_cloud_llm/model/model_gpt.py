...
from edge_cloud_llm.model.moe import MoELayer
...
class RotaryEmbedding(nn.Module):
    ...
    def get_cos_sin(self, positions, device):
        positions = positions.to(device).float()
        freqs = torch.outer(positions, self.inv_freq)
        emb = torch.cat([freqs, freqs], dim=-1)
        cos = emb.cos().unsqueeze(0).unsqueeze(0)
    ...
    def apply_rotary(self, q, k, positions):
        cos, sin = self.get_cos_sin(positions, q.device)
        q = (q * cos) + (rotate_half(q) * sin)
        k = (k * cos) + (rotate_half(k) * sin)
        return q, k
...
# ------------------------------
class CausalSelfAttention(nn.Module):
    def __init__(self, n_embd, n_head, dropout, rope):
        super().__init__()
        self.n_head = n_head
        self.d_head = n_embd // n_head
        self.rope = rope
...
class Block(nn.Module):
    def __init__(self, n_embd, n_head, dropout, rope):
        super().__init__()
        self.ln1 = RMSNorm(n_embd)
        self.attn = CausalSelfAttention(n_embd, n_head, dropout, rope)

        self.ln2 = RMSNorm(n_embd)
        self.moe = MoELayer(n_embd, num_experts=4)
    ...
    def forward(self, x):
        ...
        # moe_out, moe_loss = self.moe(self.ln2(x))
        # x = x + moe_out

        x = x + self.attn(self.ln1(x))

        dense_out = self.ffn(self.ln2(x))
        moe_out, moe_loss = self.moe(self.ln2(x))
...
class GPT(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        rope = RotaryEmbedding(n_embd // n_head, base=rope_base)

        self.blocks = nn.ModuleList([
            Block(n_embd, n_head, dropout, rope)
            for _ in range(n_layer)
        ])
    ...
    def forward(self, idx, targets=None, use_cache=False):
        ...
        if targets is not None:
            ...
            # loss = lm_loss + 0.01 * total_moe_loss
            loss = lm_loss + 0.001 * total_moe_loss
